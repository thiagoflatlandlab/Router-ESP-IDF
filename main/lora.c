...
{
   if (len > (MAX_LORA_FIFO_SIZE - 1)) {
       ESP_LOGE(TAG, "Erro: Tamanho do buffer excede o limite da FIFO");
       return;
   }

   uint8_t out[MAX_LORA_FIFO_SIZE];
   out[0] = 0x80 | reg;
...
{
   if (len > (MAX_LORA_FIFO_SIZE - 1)) {
       ESP_LOGE(TAG, "Erro: Tamanho de leitura excede o limite da FIFO");
       return;
   }

   // Alocação na Stack
   uint8_t out[MAX_LORA_FIFO_SIZE];
   uint8_t in[MAX_LORA_FIFO_SIZE];
   
   out[0] = reg;
   for (int i = 0; i < len; i++) {
      out[i + 1] = 0xff; // Bytes dummy para puxar o clock do SPI
   }
